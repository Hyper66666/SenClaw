> Alignment note: items are checked only where the current `apps/web` code, docs, or automated verification demonstrates the behavior. Manual accessibility, performance, browser, and gateway-static-hosting verification remain open.

## 1. Project Scaffold

- [x] 1.1 Create `apps/web` directory with Vite + React + TypeScript template using `pnpm create vite`.
- [x] 1.2 Add dependencies: `react-router-dom`, `@tanstack/react-query`, `tailwindcss`, `@radix-ui/react-*` primitives.
- [x] 1.3 Configure Tailwind CSS with dark mode support (`class` strategy) and custom color palette.
- [x] 1.4 Set up `tsconfig.json` extending workspace base, add path aliases (`@/` -> `src/`).
- [x] 1.5 Configure Vite dev server to proxy `/api` and `/health` to gateway (port 4100).
- [x] 1.6 Add `apps/web` to workspace references in `tsconfig.workspace.json`.
- [ ] 1.7 Update root `package.json` to add `dev:web` script (`pnpm --filter @senclaw/web dev`).

## 2. Base UI Components

- [x] 2.1 Implement `Button` component with variants (primary, secondary, ghost, danger) and loading state.
- [x] 2.2 Implement `Card` component with optional title and actions slot.
- [x] 2.3 Implement `Input` component with label, error state, and validation feedback.
- [x] 2.4 Implement `Textarea` component for multi-line input.
- [ ] 2.5 Implement `Select` component using Radix UI Select primitive.
- [x] 2.6 Implement `Badge` component with color variants.
- [ ] 2.7 Implement `LoadingSpinner` component with size variants.
- [x] 2.8 Implement `ErrorMessage` component with retry action.
...
- [x] 3.2 Define `APIError` class with status, code, and details fields.
- [x] 3.3 Implement `agentAPI` methods: `list()`, `get(id)`, `create(data)`, `delete(id)`.
- [x] 3.4 Implement `taskAPI.submit(task)` method.
- [x] 3.5 Implement `runAPI` methods: `get(id)`, `getMessages(id)`.
- [x] 3.6 Implement `healthAPI.check()` method.
- [x] 3.7 Set up TanStack Query client with default options (staleTime, retry, refetchOnWindowFocus).
- [x] 3.8 Implement `useAgents()`, `useAgent(id)`, `useCreateAgent()`, `useDeleteAgent()` hooks.
- [x] 3.9 Implement `useRun(id)` hook with automatic polling for active runs.
- [x] 3.10 Implement `useRunMessages(runId)` hook.
- [x] 3.11 Implement `useSubmitTask()` hook.
- [x] 3.12 Implement `useHealth()` hook with 30s refetch interval.
...
- [x] 4.1 Set up React Router with routes: `/` (agents list), `/agents/:id` (detail), `/agents/new` (create), `/runs` (list), `/runs/:id` (detail), `/tasks/new` (submit), `/health` (dashboard).
- [x] 4.2 Implement `AppLayout` component with header, navigation, and content area.
- [x] 4.3 Add navigation links in header: Agents, Runs, Tasks, Health.
...
- [x] 5.1 Implement `AgentListPage`: fetch agents with `useAgents()`, display in grid layout.
...
- [x] 5.4 Add "Create Agent" button in page header, navigates to `/agents/new`.
- [x] 5.5 Implement `AgentDetailPage`: fetch agent with `useAgent(id)`, display full details.
- [ ] 5.6 Show agent system prompt in expandable/collapsible section.
- [x] 5.7 Display provider config (provider name, model) in formatted layout.
- [x] 5.8 List agent tools with badges.
- [ ] 5.9 Add "Delete Agent" button with confirmation dialog (Radix UI AlertDialog).
- [x] 5.10 Implement `AgentCreatePage` with form: name (input), system prompt (textarea), provider (select), model (input), tools (multi-select or comma-separated input).
...
- [x] 5.14 Add error handling for API failures (show toast or error message).

## 6. Task Submission Page

- [x] 6.1 Implement `TaskSubmitPage` with form: agent selector (dropdown), task input (textarea).
- [x] 6.2 Fetch agents with `useAgents()` to populate dropdown.
- [x] 6.3 Add validation: agent must be selected, input must not be empty.
- [x] 6.4 Handle form submission with `useSubmitTask()`, show loading state.
- [x] 6.5 Navigate to `/runs/:id` on successful task submission.
- [x] 6.6 Display error message if submission fails.

## 7. Run Monitoring Pages

- [x] 7.1 Implement `RunListPage`: fetch runs (initially from cache, later add API endpoint for list).
...
- [x] 7.7 Implement `RunDetailPage`: fetch run with `useRun(id)`, display status, timestamps, error (if failed).
...
- [x] 7.10 Fetch messages with `useRunMessages(runId)`, display in `MessageList` component.
...
- [x] 8.1 Implement `HealthPage`: fetch health status with `useHealth()`.
- [x] 8.2 Display overall system status with large `HealthIndicator` component.
- [x] 8.3 Show individual service health (gateway, storage) if `details` are present.
- [ ] 8.4 Display last checked timestamp.
- [ ] 8.5 Add "Refresh" button to manually trigger health check.
- [x] 8.6 Show error message if health check fails.
...
- [x] 11.1 Define CSS variables for colors in `globals.css` (light and dark mode).
- [x] 11.2 Configure Tailwind to use CSS variables for theme colors.
...
- [x] 15.4 Add unit tests for API client methods (mock fetch).
...
- [x] 16.1 Add README.md to `apps/web` with setup instructions, dev commands, build commands.
...
- [x] 16.5 Update root README.md to mention web console and how to access it.

## 17. Verification

- [ ] 17.1 Run `pnpm run verify` and ensure all checks pass (lint, typecheck, tests).
- [x] 17.2 Run `pnpm run build` and verify production build succeeds.
